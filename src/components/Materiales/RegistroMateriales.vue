  ...
  <div class="ed-item ed-container sep-5">
    <el-card v-for="(hora, index) in arrHoras" :key="index" shadow="hover" class="ed-item sep-5 box-card">
      <div slot="header" class="clearfix">
        ...
        <a class="el-button el-button--primary el-button--mini" v-if="hora.img && hora.img != ''" target="_blank" :download="hora.title + '.pdf'" style="float: right" :href="`${URLIMAGE}uploads/materiales/${hora.img}`">Descargar Material</a>
      </div>
      <div class="text-item">
...
export default {
  ...
  methods: {
    ...
    async create ($hora) {
      try {
        const res = await this.axios.post(`${this.URL_}v2/materiales_asignatura/store`, $hora)
    ...
    async saveMaterial (event, id, title) {
      ...
      try {
        const res = await this.axios.post(`${this.URL}material-pdf/${id}`, data, this.autho)
