...
<div class="ed-container init">
  <h1 class="ed-item  gr-center">
    <strong>REGISTRO PLANES DE CLASES</strong>
  </h1>
  ...
  <div class="ed-item ed-container sep-5">
    <el-card v-for="(hora, index) in arrHoras" :key="index" shadow="hover" class="ed-item sep-5 box-card">
      <div slot="header" class="clearfix">
        ...
        <el-button @click="clickFile(hora)" v-if="!isEdit(hora)" plain size="mini" style="float: right" type="info">Subir Plan de Clases</el-button>
        <input @change="saveMaterial($event, hora.id, hora.title)" :id="'material-' + hora.id" type="file" class="file-hide">
        <a class="el-button el-button--primary el-button--mini" v-if="hora.img && hora.img != ''" target="_blank" :download="hora.title + '.pdf'" style="float: right" :href="`${URLIMAGE}uploads/planes_clases/${hora.img}`">Descargar Plan de Clases</a>
      </div>
      <div class="text-item">
...
export default {
  ...
  methods: {
    ...
    async create ($hora) {
      try {
        const res = await this.axios.post(`${this.URL_}v2/planes_curso/store`, $hora)
    ...
    async saveMaterial (event, id, title) {
      ...
      try {
        const res = await this.axios.post(`${this.URL}planes-clase-pdf/${id}`, data, this.autho)
