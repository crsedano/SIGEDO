    ...
    <el-table border stripe fit :data="arrCursos" height="500" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column label="*" min-width="120" align="center" header-align="center">
        <template slot-scope="reg">
          <el-button @click="goRegisterMateriales(reg.row)" size="mini" type="primary">Materiales</el-button>
        </template>
      </el-table-column>
      <template slot="empty">
...
export default {
  ...
  methods: {
    ...
    async getCursosAsignados() {
      ...
      try {
        const res = await this.axios.get(`${this.URL_}v2/materiales_asignatura/asignaturas_docente/${id_periodo}/${id_semestre}`)
