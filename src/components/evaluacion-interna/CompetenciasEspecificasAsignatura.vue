        ...
        <table class="Horario--Table">
          <thead>
            <tr>
              ...
              <th>Asignatura</th>
              <th>Ciclo</th>
              <th v-for="(competencia, i) in competenciasEspecificas" :key="`${i}d`">{{ competencia.competencia }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(asignatura, i) in asignaturas" :key="`${i}h`">
              ...
              <template v-for="(competencia, ii) in competenciasEspecificas">
                <td :key="`${i}-${ii}`">
                  <el-button @click="registerEspecificoAsignatura(competencia.id, asignatura.id_asignatura)" v-if="!getCompetenciaArea(asignatura.id_asignatura, competencia.id)" type="primary" size="mini">Marcar</el-button>
                  <span v-else>SI</span>
                </td>
              </template>
            </tr>
...
export default {
  ...
  data () {
    return {
      ...
      competenciasEspecificasAsignaturas: null,
      asignaturas: null,
  ...
  methods: {
    ...
    async loadCompetenciasAreasEspecificas () {
      ...
      try {
        const { status, data } = await this.axios.get(`${this.URL_}2/evaluacion-interna/competencias-especificos-asignaturas/${this.planId}/${this.escuelaId}`)

        if (status === 200) {
          this.competenciasEspecificasAsignaturas = data.competencias_especificos_asignaturas
          this.asignaturas = data.asignaturas
        }
      } catch (err) {
        this.competenciasEspecificasAsignaturas = []
        this.$hlp.errors(err)
      }
      loading.close()
    },
    ...
    getCompetenciaArea (asignatura_id, competencia_id) {
      let competenciaArea = this.competenciasEspecificasAsignaturas.find(h => h.asignatura_id == asignatura_id && h.competencia_id == competencia_id)

      return !!competenciaArea
    },

    async registerEspecificoAsignatura (competenciaId, asignaturaId) {
      try {
        const { status, data } = await this.axios.post(`${this.URL_}2/evaluacion-interna/register-especifico-asignatura/${ competenciaId }/${ asignaturaId }`)
