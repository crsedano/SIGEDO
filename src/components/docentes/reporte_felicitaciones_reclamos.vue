...
<div class="ed-container init">
  <h2 class="ed-item sep-10 gr-center">
    <strong>
      REPORTE FELICITACIONES RECLAMOS
    </strong>
  </h2>
  ...
  <div v-if="arrData" class="ed-item txt-center">
    <a v-if="entorno" target="_blank" :href="`${URLPDF}api/src/reportes/docentes/reporte_felicitaciones_reclamos.php?ae_bcd=${token}&id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_filial=${id_filial}&id_modalidad=${id_modalidad}&id_facultad=${id_facultad}&id_escuela=${id_escuela}&id_usuario=${this.identity.id_usuario}`"
  ...
  <div class="ed-item sep-10">
    <el-table id="table" border stripe fit :data="arrData" height="350" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column prop="tipo" label="Tipo" min-width="150" align="left" header-align="center"></el-table-column>
      <el-table-column prop="codigo" label="Codigo" min-width="150" align="left" header-align="center"></el-table-column>
      <el-table-column prop="alumno" label="Alumno" min-width="150" align="left" header-align="center"></el-table-column>
      <el-table-column prop="descripcion" label="Descripcion" min-width="150" align="left" header-align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDataReporte: async function () {
      ...
      let gxFelicitacionesReclamos = new GxFelicitacionesReclamos(this.URL)
      ...
      const res = await gxFelicitacionesReclamos.reportesdelicitacionreclamo(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela)
      if (res.success) {
        this.arrData = res.data
        if (this.id_titulo == '0') {
          this.extractFelicitacionesReclamos(this.arrData)
        }
      } else {
        this.arrData = null
    ...
    extractFelicitacionesReclamos: function (arr) {
      this.arrFelicitacioneReclamos = []

      arr.map(data => {
        if (!this.arrFelicitacioneReclamos.some(d => d.id_docente == data.id_docente)) {
          let obj = {
          ...
          this.arrFelicitacioneReclamos.push(obj)

          this.arrFelicitacioneReclamos.sort((a, b) => {
