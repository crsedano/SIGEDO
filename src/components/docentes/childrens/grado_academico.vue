...
<div class="ed-container init">
  ...
  <div class="ed-item s-95 to-center">
    <el-form class="ed-item ed-container" label-position="top" size="mini" ref="form" :model="gx_grado_academico" label-width="120px">
      ...
      <el-form-item
        ...
        prop="id_titulo"
        :rules="[
          { required: true, message: 'La denominacion es obligatoria', trigger: 'blur' }
        ]">
        <el-select v-model="gx_grado_academico.id_titulo" filterable>
          <el-option v-for="(grado, index) in grados" :key="index" :label="`En ${grado.nombre}`" :value="grado.id_titulo">
          </el-option>
        </el-select>
        <span v-if="entorno && entorno.modal == 'S'" @click="crud_grados = true" class="hint--top hint--bounce" aria-label="Administrar Denominaciones">
      ...
      <el-form-item
        v-if="tipo_grado == 2 || tipo_grado == 3 || tipo_grado == 4"
        class="ed-item sep-5 m-50 l-30 to-center" 
        label="Mención en"
        prop="cmension"
        :rules="[
        ...
        <el-input v-model="gx_grado_academico.cmension"></el-input>
      </el-form-item>

      <el-form-item
        class="ed-item sep-5 m-50 l-30 to-center" 
        label="Fecha Obtención de Grado">
    ...
    <div class="ed-item sep-10">
      ...
      >
        <el-table-column prop="ctipo" label="Grado" min-width="150" align="left" header-align="center"></el-table-column>
        <el-table-column prop="ctitulo" label="Denominación" min-width="300" align="left" header-align="center"></el-table-column>
        <el-table-column prop="cmension" label="Mención" min-width="250" align="left" header-align="center"></el-table-column>
        ...
        <el-table-column prop="fgrado" label="Obtencion Grado" min-width="200" align="left" header-align="center"></el-table-column>
        <el-table-column prop="c9" label="Mayor grado" min-width="150" align="center" header-align="center">
          <template slot-scope="reg">
            <i v-if="reg.row.c9 == 1" class="fas fa-check-circle pointer action"></i>
          </template>
        </el-table-column>
        ...
        <el-table-column label="Adjuntar" min-width="80" align="center" header-align="center">
          <template slot-scope="reg">
            <div v-if="reg.row.id_grado!='' && reg.row.id_grado!=null">
              ...
              <input @change="saveFotoGrado($event,reg.row.id_grado, reg.row.ctipo)" v-bind:id="'imgEntradaGrado'+reg.row.id_grado" type="file"
...
export default {
  ...
  computed: {
    ...
    gradoValido: function () {
      return (
        this.gx_grado_academico.id_titulo != 'SELECCIONAR' &&
        this.gx_grado_academico.id_titulo != '' &&
        this.gx_grado_academico.cmension.length<=255 &&
  ...
  methods: {
    ...
    async agregarGrado() {
      ...
      if (this.tipo_grado == 1) {
        this.gx_grado_academico.ctipo = 'Bachiller'
      } else if (this.tipo_grado == 2) {
        this.gx_grado_academico.ctipo = 'Maestro'
      } else if (this.tipo_grado == 3) {
        this.gx_grado_academico.ctipo = 'Doctor'
    ...
    actualizarMaestria: async function () {
      if (this.gradosAcemicos.some(grado => grado.ctipo == 'Maestro')) {
        this.cmaestria = 'SI'
      } else {
        this.cmaestria = 'NO'
      }
      ...
      try {
        const res = await this.axios.post(`${this.URL}js-grado-academico-update-maestria/${this.dni}`, {
          'cmaestria': this.cmaestria
        }, this.autho)
    ...
    actualizarDoctorado: async function () {
      if (this.gradosAcemicos.some(grado => grado.ctipo == 'Doctor')) {
        this.cdoctorado = 'SI'
      } else {
        this.cdoctorado = 'NO'
      }
      ...
      try {
        const res = await this.axios.post(`${this.URL}js-grado-academico-update-doctorado/${this.dni}`, {
          'cdoctorado': this.cdoctorado
        }, this.autho)
    ...
    async getGradoAcademico (id_docente) {
      ...
      try {
        const res = await this.axios.get(`${this.URL}js-grado-academico/${id_docente}`, this.autho)
    ...
    async saveFotoGrado (event, id_grado, tipo) {
      const loading = this.$loading({
        lock: true,
        text: 'Cargando...',
        spinner: 'el-icon-loading',
  ...
  watch: {
    ...
    'gx_grado_academico.id_titulo': function (n) {
      ...
      if (this.grados.filter(g => g.id_titulo == n).length > 0) {
        ...
        this.gx_grado_academico.ctitulo = ''
      }
    },
