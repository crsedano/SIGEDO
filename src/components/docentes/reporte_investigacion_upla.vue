<template>
    <div class="ed-container init">
        ...
        <div v-if="arrData" class="ed-item txt-center">
            <a v-if="entorno" target="_blank" :href="`${URLPDF}api/src/reportes/docentes/reporte_investigacion_upla.php?ae_bcd=${token}&id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_filial=${id_filial}&id_modalidad=${id_modalidad}&id_facultad=${id_facultad}&id_escuela=${id_escuela}&id_usuario=${this.identity.id_usuario}`" class="el-button el-button--info">Imprimir PDF</a>
        ...
        <div class="ed-item sep-10">
          ...
          >
            ...
            <el-table-column prop="id_docente" label="DNI" min-width="80" align="left" header-align="center"></el-table-column>
            <el-table-column prop="docente" label="Docente" min-width="200" align="left" header-align="center"></el-table-column>
            <el-table-column prop="resolucion" label="Resolución" min-width="150" align="left" header-align="center"></el-table-column>
            <el-table-column prop="titulo" label="Titulo" min-width="150" align="left" header-align="center"></el-table-column>
            <el-table-column prop="fecha_inicio" label="Inicio" min-width="80" align="left" header-align="center"></el-table-column>
            <el-table-column prop="fecha_fin" label="Fin" min-width="80" align="left" header-align="center"></el-table-column>
            ...
            <table id="table" class="table table-responsive table-striped table-bordered table-hover table-own">
                ...
                <tbody class="table-body">
                    <tr v-for="(data, index) in arrData" :key="index">
                        ...
                        <td style="text-align: left">{{ data.resolucion }}</td>
                        <td style="text-align: left">{{data.titulo}}</td>
                        <td style="text-align: left">{{ data.fecha_inicio }}</td>
                        <td style="text-align: left">{{ data.fecha_fin }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
export default {
    ...
    methods: {
        ...
        getDataReporte: async function() {
            ...
            let gxInvestigacionUpla = new GxInvestigacionUpla(this.URL)
            ...
            const res = await gxInvestigacionUpla.reportesinvestigacion(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela)
            if(res.success) {
                this.arrData = res.data
                  if (this.id_titulo == '0') {
                      this.extractInvestigacionUpla(this.arrData)
                  }
            } else {
                this.arrData = null
        ...
        extractInvestigacionUpla: function(arr) {
            this.arrInvestigacionUpla = []

            arr.map(data => {
                if(!this.arrInvestigacionUpla.some(d => d.id_docente == data.id_docente)) {
                    let obj = {
                    ...
                    this.arrInvestigacionUpla.push(obj)

                    this.arrInvestigacionUpla.sort((a, b) => {
                        return parseInt(a.id_docente) - parseInt(b.id_docente)
                    })
                }
            })
        },
         ...
         exportToExcel: function (isFixed = true) {
          ...
          if (tbl) {
            ...
            XLSX.writeFile(
              wb,
              "reporte_investigaciones upla.xlsx", {
                compression: true
              }
            );
          }
