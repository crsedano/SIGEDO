  ...
  <div class="ed-item ed-container sep-5">
    <el-card v-for="(crono, index) in cronograma" :key="index" shadow="hover" class="ed-item sep-5 box-card">
      ...
      <div class="text item es-item">
        ...
        <textarea disabled  style="width: 100%; border: none" rows="5" v-model="crono.actividades"></textarea>
        <label>Operaciones</label>
        <textarea disabled  style="width: 100%; border: none" rows="5" v-model="crono.operaciones"></textarea>
      </div>
  ...
  methods: {
    ...
    async getCronograma () {
      try {
        ...
        const res = await this.axios.get(`${this.URL_}v2/avance_silabo/programa/${id_periodo}/${id_semestre}/${this.idCurso}`)
    ...
    createCronograma (cronograma) {
      let arr = cronograma.map(crono => {
        let done = false
        if (crono.semana * 2 <= this.porcentaje) {
          done = true
        }
    ...
    async setDone({ semana }) {
      try {
        const res = await this.axios.put(`${this.URL_}v2/avance_silabo/set_porcentaje/${this.idAsigdoc}`, {porcentaje: semana * 2})
