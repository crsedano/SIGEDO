...
<div class="ed-container init">
  <h1 class="ed-item  gr-center">
    <strong>REPORTE AVACE SILABO</strong>
  </h1>
  ...
  <el-form size="mini" class="ed-item sep-5 ed-container" label-width="120px">
    ...
    <el-form-item class="ed-item m-50" label="Filial">
      <el-select filterable v-model="idFilial">
        <el-option v-for="(filial, index) in filiales" :key="index" :label="filial.nombre_filial"
          :value="filial.id_filial"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item class="ed-item m-50" label="Facultad">
      <el-select filterable v-model="idFacultad">
        <el-option v-for="(facultad, index) in facultades" :key="index" :label="facultad.nombre"
          :value="facultad.id_facultad"></el-option>
      </el-select>
    </el-form-item>
    <el-form-item class="ed-item m-50" v-if="facultadSelected" label="Escuela">
      <el-select filterable v-model="idEscuela">
        <el-option v-for="(escuela, index) in escuelas" :key="index" :label="escuela.nombre"
          :value="escuela.id_escuela"></el-option>
      </el-select>
    </el-form-item>
    ...
    <el-form-item class="ed-item txt-center">
      ...
      <a v-if="dataReport"
        :href="`${URLPDF}api/src/reportes/avance_silabo?ae_bcd=${token}&idUsuario=${identity.id_usuario}&idPeriodo=${periodo.id_periodo}&idSemestre=${periodo.id_semestre}&idFilial=${idFilial}&idFacultad=${idFacultad}&idEscuela=${idEscuela}&idModalidad=${idModalidad}&idDocente=${idDocente}`"
        target="_blank" class="el-button el-button--info el-button--mini" rel="noopener noreferrer">Imprimir</a>
    </el-form-item>
  </el-form>

  <div class="ed-item sep-10">
    <el-table border stripe fit :data="dataReport" height="350" highlight-current-row
      :row-class-name="tableRowClassName" v-loading="ajaxLoad" element-loading-spinner="el-icon-loading"
      element-loading-background="rgba(0, 0, 0, 0.8)" element-loading-text="Cargando cursos a cargo...">
      <el-table-column fixed type="index" :index="indexMethod" min-width="40" align="center" header-align="center">
      </el-table-column>
      <el-table-column prop="id_docente" label="DNI" min-width="80" align="center" header-align="center">
      </el-table-column>
      <el-table-column prop="nombre" label="Docente" min-width="250" align="left" header-align="center">
      </el-table-column>
      <el-table-column prop="asignatura" label="Asignatura" min-width="200" align="center" header-align="center">
      </el-table-column>
      <el-table-column prop="anio" label="Plan" min-width="70" align="center" header-align="center"></el-table-column>
      <el-table-column prop="facultad" label="Facultad" min-width="150" align="center" header-align="center">
      </el-table-column>
      <el-table-column prop="escuela" label="Escuela" min-width="180" align="center" header-align="center">
      </el-table-column>
      <el-table-column sortable prop="porcentaje" label="%Avance" min-width="100" align="center" header-align="center">
      </el-table-column>
    </el-table>
  </div>
  ...
  methods: {
    ...
    async getIncDocente() {
      try {
        ...
        const res = await this.axios.get(`${this.URL_}v2/avance_silabo/reporte/${this.page}/${this.limit}/${id_periodo}/${id_semestre}/${this.idFilial}/${this.idFacultad}/${this.idEscuela}/${this.idModalidad}/${this.idDocente}`)
        if (res.status !== 200) return
    ...
    async getWoDocente() {
      try {
        ...
        const res = await this.axios.get(`${this.URL_}v2/avance_silabo/reporte/${this.page}/${this.limit}/${id_periodo}/${id_semestre}/${this.idFilial}/${this.idFacultad}/${this.idEscuela}/${this.idModalidad}`)
        if (res.status !== 200) return
        this.dataReport = res.data.data
        this.setTotal(res.data.total)
      } catch (err) {
