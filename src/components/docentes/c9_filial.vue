<template>
  <div class="ed-container init">
    <h3 class="ed-item gr-center">
      <strong>REPORTE C9 CON FILIAL</strong>
    </h3>
    ...
    <div v-if="estadoFacultad" class="ed-item txt-center">
      ...
      <a
        ...
        target="_blank"
        :href="`${URLPDF}api/src/reportes/c9/c10.php?id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_facultad=${id_facultad}&id_cargo=${estadojefepract}&token=${token}&id_usuario=${identity.id_usuario}&id_modalidad=${this.id_modalidad}&id_filial=${this.id_filial}&id_docente=${this.id_docente}&id_escuela=${this.id_escuela}&dedicacion=${this.dedicacion}`"
      >
      ...
      <table class="main-table">
        ...
        <tbody class="table-body">
          <tr v-for="(user, index) in estadoFacultad" :key="index">
            ...
            <td class="w-30">{{ user.hAdmi }}</td>
            <td class="w-30">{{ user.totalHoras }}</td>
            <td class="w-30">{{user.investigador}}</td>
            <td class="w-30">{{user.dina}}</td>
            <td class="w-30">{{user.facultad_adscrita}}</td>
            <td class="w-200">{{user.nombre_filial}}</td>
            <td>{{ user.is_jp == 1 ? 'JEFE PRÁCTICA' : 'DOCENTE'}}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getEstadoFacultad() {
      ...
      try {
        const res = await this.axios.get(
          `${this.URL_}v2/c9/data/${this.periodo.id_periodo}/${
            this.periodo.id_semestre
          }/${this.id_facultad}/${!this.estadojefepract}/${this.id_modalidad}/${
            this.id_filial
          }/${this.id_docente}/${this.id_escuela}/${this.dedicacion}`
        );
        console.log(res);
        if (res.status === 200) {
          this.estadoFacultad = res.data.docentes;
        }
      } catch (err) {
    ...
    //   try {
    //     const res = await this.axios.get(`${this.URL}js-c9-facultad/${this.periodo.id_periodo}/${this.periodo.id_semestre}/${this.id_facultad}/${this.estadojefepract}/${this.id_modalidad}/${this.id_filial}/${this.id_docente}/${this.id_escuela}/${this.dedicacion}`)
