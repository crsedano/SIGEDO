...
<!-- v-if="ready"  -->
  ...
  <div v-if="arrData" class="ed-item gr-center">
    ...
    <a v-if="entorno" target="_blank" :href="`${URLPDF}api/src/reportes/docentes/reporte_bachiller.php?ae_bcd=${token}&id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_filial=${id_filial}&id_modalidad=${id_modalidad}&id_facultad=${id_facultad}&id_escuela=${id_escuela}&id_usuario=${this.identity.id_usuario}`"
  ...
  <div id="table" v-if="arrData" class="ed-item sep-10">
    <el-table border stripe fit :data="arrData" height="350" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column prop="titulo" label="Bachiller" min-width="90" align="center" header-align="center"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDataReporte: async function () {
      ...
      const res = await gxTitulo.reporteBachiller(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela, this.d)

      if (res.success) {
        this.arrData = res.data

        if (this.id_titulo == '0') {
          this.extractBachiller(this.arrData)
        }
      } else {
        this.arrData = null
    ...
    extractBachiller: function (arr) {
      this.arrBachiller = []

      arr.map(data => {
        if (!this.arrBachiller.some(d => d.id_titulo == data.id_titulo)) {
          let obj = {
            id_titulo: data.id_titulo,
            titulo: data.titulo
          }

          this.arrBachiller.push(obj)

          this.arrBachiller.sort((a, b) => {
