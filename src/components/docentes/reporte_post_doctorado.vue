...
<div class="ed-container init">
  ...
  <div v-if="arrData" class="ed-item gr-center">
    ...
    <a v-if="entorno" target="_blank" :href="`${URLPDF}api/src/reportes/docentes/reporte_post_doctorado.php?ae_bcd=${token}&id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_filial=${id_filial}&id_modalidad=${id_modalidad}&id_facultad=${id_facultad}&id_escuela=${id_escuela}&id_usuario=${this.identity.id_usuario}`"
      class="btn btn-dark" type="primary">Imprimir PDF</a>

    <el-button @click="exportToExcel" class="btn btn-dark" type="primary">
      Exportar a Excel
    </el-button>
  ...
  <div id="table" v-if="arrData" class="ed-item sep-10">
    <el-table border stripe fit :data="arrData" height="350" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column prop="postdoctorado" label="Doctorado" min-width="90" align="center" header-align="center"></el-table-column>
...
import GxPostDoctorado from '../../models/gx_postdoctorado'
...
export default {
  ...
  methods: {
    ...
    getDataReporte: async function () {
      ...
      gxPostDoctorado.id_titulo = this.id_titulo
      ...
      const res = await gxPostDoctorado.reportePostDoctorado(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela, this.d)
      if (res.success) {
    ...
    extractDoctorado: function (arr) {
      this.arrPostDoctorado = []

      arr.map(data => {
        if (!this.arrPostDoctorado.some(d => d.id_postdoctorado == data.id_postdoctorado)) {
          let obj = {
            id_postdoctorado: data.id_postdoctorado,
            postdoctorado: data.postdoctorado
          }

          this.arrPostDoctorado.push(obj)

          this.arrPostDoctorado.sort((a, b) => {
            return parseInt(a.id_postdoctorado) - parseInt(b.id_postdoctorado)
          })
        }
      })
    },
