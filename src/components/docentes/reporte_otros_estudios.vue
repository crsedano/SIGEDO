...
<div class="ed-container init">
  ...
  <div class="ed-item ed-container">
    <div class="ed-item sep-5 m-50 to-center form-group">
      ...
      <select v-model="idvar" class="form-control">
        <option v-for="(tipo, index) in arrTiposEstudio" :key="index" :value="tipo.idvar">
          {{tipo.cnombre}}
        </option>
      </select>
    </div>
    ...
    <div v-if="arrEscuelas" class="ed-item sep-5 m-50 to-center form-group">
  ...
  <div class="ed-item sep-10">
    <el-table id="table" border stripe fit :data="arrData" height="350" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column prop="nombre_titulo" :label="type" min-width="150" align="left" header-align="center"></el-table-column>
...
export default {
  ...
  computed: {
    ...
    type: function () {
      if (!this.arrTiposEstudio) return ''

      let name = this.arrTiposEstudio.find(tipo => tipo.idvar == this.idvar).cnombre
  ...
  methods: {
    ...
    getDataReporte: async function () {
      ...
      const res = await gxOtrosEstudios.reportes(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela, idvar)
