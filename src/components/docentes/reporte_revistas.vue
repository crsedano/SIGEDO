...
<div class="ed-container init">
  ...
  <div v-if="arrData" class="ed-item txt-center">
    <a v-if="entorno" target="_blank" :href="`${URLPDF}api/src/reportes/docentes/reporte_revistas.php?ae_bcd=${token}&id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_filial=${id_filial}&id_modalidad=${id_modalidad}&id_facultad=${id_facultad}&id_escuela=${id_escuela}&id_usuario=${this.identity.id_usuario}`"
  ...
  <div class="ed-item sep-10">
    <el-table id="table" border stripe fit :data="arrData" height="350" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column prop="revista" label="Revista" min-width="150" align="left" header-align="center"></el-table-column>
      <el-table-column prop="titulo" label="Titulo" min-width="150" align="left" header-align="center"></el-table-column>
      <el-table-column prop="fecha_publicacion" label="Publicacion" min-width="80" align="left" header-align="center"></el-table-column>
            ...
            <table id="table" class="table table-responsive table-striped table-bordered table-hover table-own">
                ...
                <tbody class="table-body">
                    <tr v-for="(data, index) in arrData" :key="index">
                        ...
                        <td class="w-250" style="text-align: left">{{ data.docente }}</td>
                        <td class="w-10">{{ data.revista }}</td>
                        <td class="w-250" style="text-align: left">{{ data.titulo }}</td>
                        <td class="w-10">{{ data.fecha_publicacion }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
import GxPublicacionRevistas from '../../models/gx_publicaciones_revistas';
...
export default {
  ...
  methods: {
    ...
    getDataReporte: async function () {
      ...
      let gxPublicacionRevistas = new GxPublicacionRevistas(this.URL)
      ...
      const res = await gxPublicacionRevistas.reportes(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela)
    ...
    exportToExcel: function (isFixed = true) {
      ...
      if (tbl) {
        ...
        XLSX.writeFile(
          wb,
          "reporte revistas indexadas.xlsx", {
            compression: true
          }
        );
      }
