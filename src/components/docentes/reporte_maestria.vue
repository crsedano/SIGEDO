...
<div class="ed-container init">
  ...
  <div v-if="arrData" class="ed-item gr-center">
    ...
    <a v-if="entorno" target="_blank" :href="`${URLPDF}api/src/reportes/docentes/reporte_maestria.php?ae_bcd=${token}&id_periodo=${periodo.id_periodo}&id_semestre=${periodo.id_semestre}&id_filial=${id_filial}&id_modalidad=${id_modalidad}&id_facultad=${id_facultad}&id_escuela=${id_escuela}&id_usuario=${this.identity.id_usuario}`"
      class="btn btn-dark ">Imprimir PDF</a>
  ...
  <div v-if="arrData" class="ed-item sep-10">
    <el-table id="table" border stripe fit :data="arrData" height="350" highlight-current-row :row-class-name="tableRowClassName"
      element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)" element-loading-text="Cargando Contactos...">
      ...
      <el-table-column prop="maestria" label="Maestria" min-width="150" align="left" header-align="center"></el-table-column>
...
import GxMagister from '../../models/gx_magister'
...
export default {
    ...
    methods: {
        ...
        getDataReporte: async function() {
            ...
            let gxMagister = new GxMagister(this.URL)
            gxMagister.id_titulo = this.id_titulo
            ...
            const res = await gxMagister.reporteMaestria(this.autho, id_periodo, id_semestre, id_filial, id_modalidad, id_facultad, id_escuela, this.d)

            if(res.success) {
                this.arrData = res.data
                if (this.id_titulo == '0') {
                    this.extractMaestrias(this.arrData)
                }
        ...
        extractMaestrias: function(arr) {
            this.arrMaestrias = []

            arr.map(data => {
                if(!this.arrMaestrias.some(d => d.id_maestria == data.id_maestria)) {
                    let obj = {
                        id_maestria: data.id_maestria,
                        maestria: data.maestria
                    }

                    this.arrMaestrias.push(obj)

                    this.arrMaestrias.sort((a, b) => {
                        return parseInt(a.id_maestria) - parseInt(b.id_maestria)
                    })
                }
            })
        },
