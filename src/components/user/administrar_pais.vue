...
<div class="ed-container init sub-main">
  ...
  <div class="ed-item sep-10 ed-container gr-center">
    <h3 class="ed-item sep-10 gr-center">
      ...
      <div v-else class="ed-item m-70 sep-10 to-center ed-container sep-10 table-content">
          <table class="table table-bordered table-responsive table-hover table-striped table-own">
            ...
            <tbody class="table-body">
              <tr v-for="(pais, index) in paises" :key="index">
                <td class="w-40">{{pais.cpais}}</td>
                <td class="w-10">{{pais.nacionalidad}}</td>
                <td :style="pais.cactivo != 1 ? 'color: red' : ''">
                  {{pais.cactivo == 1 ? 'Activo' : 'Inactivo'}}
                </td>
                <td @click="goEditPais(pais)" class="hint--left hint--info" :aria-label="`Editar País`">
...
import crudPais from './modals/crudPais.vue'
