  ...
  <div class="ed-container init">
    <div class="ed-item">
      <h1 class="text-center">
        <strong>HISTORIAL DESEMPEÑO DOCENTE</strong>
      </h1>
    </div>

    <div class="ed-item text-center">
      <el-button @click="dialog.findDocente = true" size="mini" type="primary">Buscar Docente</el-button>
      <el-button v-if="docente" @click="dialog.historialDesempenioAdmin = true" size="mini" type="primary">Registrar Desempenio</el-button>
    </div>
    ...
    <el-dialog
      ...
      :visible.sync="dialog.historialDesempenioAdmin"
      width="70%"
    >
      <historial-desempenio-admin 
        :idDocente="docente.id_docente" 
        v-if="dialog.historialDesempenioAdmin && docente" 
        @close="dialog.historialDesempenioAdmin = false"
...
<script>
import HistorialDesempenioAdmin from './dialog/HistorialDesempenioAdmin.vue'
...
export default {
  ...
  components: { FindDocente, HistorialDesempenioAdmin },
  ...
  data() {
    return {
      ...
      dialog: {
        findDocente             : false,
        historialDesempenioAdmin: false
      },
  ...
  methods: {
    selectDocente (docente) {
      ...
      this.historial = docente.desempenios
    },
    ...
    async removeHistorial ({ id }) {
      try {
        const { status } = await this.axios.post(`${this.URL_}v2/historial-desempenio/delete/${id}`)
