...
<div class="ed-container init">
  <div class="ed-item txt-center">
    <h1>
      <strong>DOCENTE INVESTIGADOR UPLA POR FACULTAD</strong>
    </h1>
  </div>
  ...
  <el-form size="mini" class="ed-item sep-5 ed-container" label-width="120px">
    ...
    <el-form-item class="ed-item txt-center">
      ...
      <a v-if="arrDocentes" :href="`${URLPDF}api/src/reportes/InvestigadorFacultad?ae_bcd=${token}&idUsuario=${identity.id_usuario}&idFacultad=${idFacultad}&idDocente=${idDocente}`" target="_blank" class="el-button el-button--info el-button--mini" rel="noopener noreferrer">Imprimir</a>
    </el-form-item>
  </el-form>
    ...
    <el-table border stripe fit :data="arrDocentes" height="350" highlight-current-row :row-class-name="tableRowClassName"
      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          <div class="txt-center inv" v-for="(inv, index) in props.row.investigaciones" :key="index">
            <h6><strong>{{ inv.ctitulo }}</strong></h6>
            <p>Resolución: {{ inv.cresolucion }}</p>
          </div>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getDocentes () {
      ...
      try {
        const res = await this.axios.get(`${this.URL_}v2/docente_investigador/by_facultad_adscrita/${this.idFacultad}${$query}`)
        if(res.status !== 200) {
          this.ajaxLoad = false
          return
        }
